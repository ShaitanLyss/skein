...
| `spawn.md` | a card putting a card on the wall and taking it off again: which territories it may name and why a path is not one of them, why the guard is branching rather than depth, the one birth path, what a card may close, and the root a spawned card is drawn on | `spawn.rs`, `lineage.ts`, `Lineage.svelte` |
