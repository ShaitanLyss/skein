<script lang="ts">
  ...
  /* ── the rails ────────────────────────────────────────────────────────────
     ...
     opening words, its headings (written as `#` or run in as a paragraph's bold
     opening, which is how most of them arrive) and the start of each item of
     each of its lists. A table of contents for a dozen answers at once is not a
     table of contents; it is the transcript again, in a narrower column.
    ...
    >
      ...
      {#if conv.history.length}
        ...
        {#each conv.history as line, i (i)}
          ...
          {:else}
            <!-- `data-nav` is what the rail collects; the line is drawn exactly
                 as it was. `pre-wrap` here, so the text stays glued to its tags. -->
      ...
      {#each conv.lines as line, i (i)}
        ...
        {:else}
          <!-- A prompt is drawn when you send it, so its own line carries whether
               the agent has it yet: dimmed while it is on its way, and saying so
...
<style>
  .detail {
    ...
    /* A flex item will not shrink below its min-content width unless told to,
       and a code fence's min-content is the length of its longest line — capped
       at `.line`'s 78ch, which is wider than this column ever is. So an answer
       containing a fence widened the whole panel past `.side` and off the right
       edge of the window, instead of scrolling inside the `pre` that is there
       to scroll. The `.meta-bar`'s mono model id does the same at the 300px
       floor. Nothing in here may set the panel's width. */
    min-width: 0;
    display: flex;
